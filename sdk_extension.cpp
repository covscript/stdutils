...
#define CS_RAISE_SIGINT() cs::global_signals.raise_sigint()
#else
#define CS_RAISE_SIGINT() cs::current_process->raise_sigint()
#endif
...
{
	const char msg[] = "Keyboard Interrupt (Ctrl+C Received)\n";
...
class repl_instance final
{
	...
	repl_instance(const cs::array &args) : context(cs::create_context(args)), repl_impl(context)
	{
	...
	bool has_exited() const { return exit_flag; }
...
CNI_ROOT_NAMESPACE
{
	...
	CNI_NAMESPACE(typeids){
	    CNI_VALUE_CONST(type, type_id(typeid(cs::type_t))) CNI_VALUE_CONST(range, type_id(typeid(cs::range_type)))
			...
			CNI_VALUE_CONST(context, type_id(typeid(cs::context_t)))
				...
				CNI_VALUE_CONST(memberfn, type_id(typeid(cs::object_method)))
					...
					CNI_VALUE_CONST(namespace, type_id(typeid(cs::namespace_t)))
						...
						type_id get_real(const var &val){return type_id(val.type());
}

CNI(get_real)
}

CNI_NAMESPACE(variable){bool is_rvalue(const var &val){return val.is_rvalue();
}
...
CNI_NAMESPACE(function){const var & get_callable(const cs::object_method &om){return om.callable;
}
...
template <typename T> var predict_domain(T &&domain)
{
